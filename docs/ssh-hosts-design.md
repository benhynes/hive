...
3. **Ensure binary** — is a compatible `hive` on the remote PATH / `--dest`?
   If not, ship it (self-copy on matching platform, else cross-compile from
   `--src`) — reuse node install's shipping logic.
...
The origin hub owns lifecycle. Options (make it a flag / profile setting):

- **ephemeral** (default): when the last agent on the SSH host is killed, stop
  the transient daemon and close the tunnels.
- **warm**: keep the daemon + tunnels up for the session (faster subsequent
  spawns), reap on origin-daemon shutdown or an idle timeout.
...
- **context files** — write the listed context into `cwd` (or `~/.claude/`), so
  ...
  `hive_send`/`hive_recv`.
- **MCP registration** — write a project-scoped `.mcp.json` in `cwd` (Claude
  Code reads it) listing the profile's MCP servers. `hive` is always included;
  since the agent's env already carries `HIVE_*`, `hive mcp` authenticates with
  zero extra config. This finally **automates** what is today a manual
  `claude mcp add hive -- hive mcp` step — and it's the natural home for adding
  *other* MCP servers per the directive.
...
- `internal/hub/` (NEW `sshhosts.go`) — the SSH-host manager owned by the hub:
  transient-daemon bring-up, tunnel lifecycle + health, teardown policy. The
  spawn path checks "is target an SSH host?" and, if so, ensures the host up
  then forwards the spawn over the tunnel.
- `internal/hub/provision.go` (NEW) — apply a spawn profile over SSH: cwd/repo,
  context files, `.mcp.json`, secrets hook. Reused by local spawns too (MCP
  auto-registration for everyone).
...
## 8. Open decisions (need a call before integration)

1. **Orchestrator: hub or CLI?** Recommended: the **hub** owns SSH-host lifecycle
   (so control ops from any client route transparently and tunnels survive the
   spawning CLI process exiting). Alternative: CLI-orchestrated (simpler, but
   tunnels die with the CLI and only that client can drive the agent).
2. **Default lifecycle** — ephemeral (tidy, slower re-spawn) vs warm (faster,
   lingering resources). Recommend ephemeral default, `--warm` opt-in.
3. **Profiles location/format** — per-net inline vs `~/.hive/profiles/*.json`
   shared across nets. Recommend shared files, referenced by name.
4. **MCP auto-registration for ALL spawns** — should local/tailnet spawns also
   auto-write `.mcp.json` with `hive`? Recommend yes (removes the manual step
   everywhere); gate behind the profile so it's opt-out-able.
5. **Model B** — commit to Model A only for now, or keep the control-backend
   abstraction seams in from the start so Model B is a later add rather than a
   refactor? Recommend leaving seams (a `control.Backend` interface) but
   implementing only tunneled-daemon.

## 9. Phased plan

- **P1 — registration + provisioning (no SSH host yet):** spawn profiles +
  `.mcp.json`/context auto-provisioning applied to **local** spawns. Delivers the
  "appropriate context and MCP servers" half immediately, testable without SSH.
- **P2 — SSH host bring-up:** `hosts add-ssh`, transient daemon over SSH, tunnel
  manager, forward-spawn. Reuse node.go shipping. e2e against an OrbStack/tart VM.
- **P3 — lifecycle + control transparency:** teardown policies, health/reconnect,
  audit; confirm read/keys/kill route through the tunnel from a third-party
  client.
...
manager (transient daemon + tunnel lifecycle), the profile provisioner
(`.mcp.json` + context + repo), and CLI surface (`hosts add-ssh`, `--profile`).
